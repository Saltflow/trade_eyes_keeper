...
def test_evaluate_percentile_basic():
    ...
    print(
        "OK: total_trades=%d, excess_return=%.2f%%, max_dd=%.2f%%, sharpe=%.4f"
        % (
            stats.total_trades,
            stats.test_excess_return,
...
class TestExecutionParams:
    def test_execution_params_decodes(self):
        ...
        p = Params(
            values=_pct_params(
                buy_score_thresh=9, sell_score_thresh=0, position_frac=4
            ),
            _engine="percentile",
        )
...
class TestScoreTimeseries:
    ...
    def test_all_min_weight_still_scores(self):
        ...
        b, s = PercentileSignalFn().score_timeseries(Params(values=p), _mk_hist())
...
class TestScanSignals:
    ...
    def test_scan_empty_history_no_crash(self):
        ...
        assert (
            PercentileSignalFn().scan_signals(Params(values=_pct_params()), {}, None)
            == []
        )
