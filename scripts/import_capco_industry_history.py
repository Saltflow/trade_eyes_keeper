#!/usr/bin/env python3
"""Import official CAPCO half-year industry PDFs into dated history."""
...
def _read_manifest(path: Path) -> tuple[str, list[dict[str, Any]]]:
    ...
    if not isinstance(documents, list) or not documents:
        raise ValueError("CAPCO source manifest must contain documents")
    required = {"period_end", "published_at", "source_url"}
    for item in documents:
        missing = required - set(item)
        if missing:
            raise ValueError(f"CAPCO source document missing: {sorted(missing)}")
    return str(payload.get("taxonomy") or "capco-listed-company-2024"), documents
...
def main() -> int:
    ...
    taxonomy, documents = _read_manifest(Path(args.source_manifest))
    cache_dir = Path(args.cache_dir) if args.cache_dir else None
    if cache_dir is not None:
        cache_dir.mkdir(parents=True, exist_ok=True)
    session = requests.Session()
    session.headers.update({
        "User-Agent": "trade-eyes-keeper research (official CAPCO taxonomy import)"
    })
    observations = []
    audit = []
    for index, document in enumerate(documents):
        ...
        cache_path = (
            cache_dir / f"{document['period_end']}.pdf"
            if cache_dir is not None
            else None
        )
        if cache_path is not None and cache_path.exists():
            content = cache_path.read_bytes()
            resolved_url = str(document["source_url"])
        else:
    ...
    payload = {
        ...
        "taxonomy": taxonomy,
        "source_kind": "official_capco_half_year_publications",
        "publication_rule": (
