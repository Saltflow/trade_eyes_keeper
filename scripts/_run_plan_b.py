"""Plan B: Position-Target model (bullish_score → sigmoid → target position)."""
import sys, time, yaml
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
import pandas as pd, numpy as np

# Force position_target mode in config
...
print(f"Plan B (Position-Target): {len(stocks_data)} stocks, 20000 samples")
...
print(f"\n{'='*62}")
print(f"Plan B (Position-Target) | Elapsed: {elapsed:.0f}s | Top: {len(report.top_strategies)}")
print(f"{'='*62}")
...
# Restore frac mode for normal use
