...
class SchedulerManager:
    ...
    def add_immediate_job(self, func, *args, **kwargs):
        """
        添加立即执行的任务
        ...
        if self.scheduler:
            try:
                self.scheduler.add_job(
                    ...
                    args=args,
                    kwargs=kwargs,
                    id=f'immediate_task_{datetime.now().timestamp()}'
                )
                logger.info("立即执行任务已添加")
                return True
            except Exception as e:
                logger.error(f"添加立即执行任务失败: {e}")
                return False
        else:
    ...
    def modify_job_time(self, hour, minute):
        """
        修改任务执行时间
        ...
        try:
            ...
            self.scheduler.reschedule_job(
                job_id='daily_stock_task',
                trigger=new_trigger
            )
            
            logger.info(f"任务执行时间已修改为 {hour:02d}:{minute:02d}")
            return True
