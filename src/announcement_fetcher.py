...
class AnnouncementFetcher:
    ...
    def _generate_sample_announcements(self, stock_code, exchange):
        """
        生成模拟公告数据（用于演示或API不可用时）
        ...
        sample_titles = [
            f"{stock_code} 2023年年度报告",
            f"{stock_code} 2023年度利润分配方案公告（每10股派发现金红利2元）",
            f"{stock_code} 关于召开2024年第一次临时股东大会的通知",
            f"{stock_code} 关于董事、监事和高级管理人员持股变动的公告",
            f"{stock_code} 关于重大项目中标的公告",
            f"{stock_code} 2023年度社会责任报告",
            f"{stock_code} 关于使用闲置自有资金进行现金管理的公告",
            f"{stock_code} 2024年第一季度业绩预告：净利润预计增长15%-25%",
            f"{stock_code} 关于股东权益变动的提示性公告",
            f"{stock_code} 关于公司债券付息的公告"
        ]
        
        sample_types = ['年报', '分红', '股东大会', '高管变动', '项目中标', '社会责任', '现金管理', '业绩预告', '股东权益', '债券付息']
        ...
        for i in range(random.randint(2, 5)):
            ...
            announcement_date = (datetime.now() - timedelta(days=days_ago)).strftime('%Y-%m-%d')
            
            title_idx = i % len(sample_titles)
            title = sample_titles[title_idx]
            ann_type = sample_types[title_idx]
            ...
            announcements.append({
                ...
                'url': f'http://example.com/announcement/{stock_code}/{announcement_date}',
                'type': ann_type,
                'summary': f"这是{stock_code}的{ann_type}相关公告，发布于{announcement_date}。"
            })
        
        logger.info(f"为股票 {stock_code} 生成了 {len(announcements)} 条模拟公告")
        return announcements