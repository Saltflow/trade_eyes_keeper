        ...
        for _, row in stock_data.iterrows():
            ...
            if close_price is not None and low_price is not None:
                if close_price < low_price:
                    logger.warning(
                        f"股票 {stock_code} 价格关系异常: 收盘价 {close_price:.2f} < 最低价 {low_price:.2f}"
                    )
                # 允许相等（跌停板）
            if close_price is not None and high_price is not None:
                if close_price > high_price:
                    logger.warning(
                        f"股票 {stock_code} 价格关系异常: 收盘价 {close_price:.2f} > 最高价 {high_price:.2f}"
                    )
                # 允许相等（涨停板）
            if low_price is not None and high_price is not None:
                if low_price > high_price:
                    logger.warning(
                        f"股票 {stock_code} 价格关系异常: 最低价 {low_price:.2f} > 最高价 {high_price:.2f}"
                    )
            ...
            if low_price < ma60:
                ...
                logger.info(
                    f"股票 {stock_code} 满足条件: 最低价 {low_price:.2f} < MA60 {ma60:.2f}"
                )
            else:
                logger.info(
                    f"股票 {stock_code} 不满足条件: 最低价 {low_price:.2f} >= MA60 {ma60:.2f}"
                )
