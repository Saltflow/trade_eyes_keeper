...
class HealthHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _load_template(self, name):
        ...
        template_path = Path(__file__).parent.parent.parent / 'templates' / 'health_server' / name
    ...
    def _send_management_page(self, error_message=None, success_message=None):
        ...
        # 生成股票表格行
        if current_stocks:
            context['stocks_html'] = ''.join(f'<tr><td>{stock}</td><td><form method="POST" action="/update-watchlist" style="display: inline;"><input type="hidden" name="action" value="remove"><input type="hidden" name="stock_code" value="{stock}"><button type="submit" class="button button-remove">移除</button></form></td></tr>' for stock in current_stocks)
        else:
