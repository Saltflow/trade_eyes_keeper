...
def _build_deep_value(indicator, threshold_norm):
    ...
    slope = (ma200 - np.roll(ma200, 20, axis=0)) / \
        np.maximum(np.roll(ma200, 20, axis=0), 1e-6)
    slope[:20] = 0
...
@register_strategy("builder")
class BuilderStrategy(ArraySignalStrategy):
    """条件构建器搜参策略。"""
    ...
    def __init__(self):
        ...
        for i in range(5):
            ...
            dims.append(ParamDim(f"buy_{i+1}_threshold",
                        THRESHOLD_LEVELS_BUILDER, 0, 1))
        for i in range(3):
            dims.append(ParamDim(f"sell_{i+1}_name", SELL_BUILDER_COUNT, 0, 1))
            dims.append(ParamDim(f"sell_{i+1}_threshold",
                        THRESHOLD_LEVELS_BUILDER, 0, 1))
    ...
    def to_human_readable(self, params: Params) -> str:
        ...
        sell_names = list(CONDITION_BUILDERS_FAST.keys())[
            BUILDER_COUNT:BUILDER_COUNT + SELL_BUILDER_COUNT]
        vals = params.values
