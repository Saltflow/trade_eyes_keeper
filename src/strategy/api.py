"""Stable contracts for trading-strategy plugins.

Strategies produce a TradePlan and never choose candidate parameters, execute
trades, calculate returns, or inspect holdout results. Search algorithms live
under ``src.search.solvers`` and consume only the typed parameter schema.
"""
...
class TradingStrategy(ABC):
    """Investment decision plugin independent of Solver and Backtester.

    A concrete plugin declares its parameter space and implements
    ``make_signals`` plus a human-readable description. Optional batch
    preparation is capability-discovered by the evaluation service.
    """
    ...
    def make_signals(
        ...
        """Return the complete decision plan consumed by every caller."""
        ...

    def _make_array_plan(
        self, params: Params, market_data: StrategyMarketData
    ) -> TradePlan:
        ...
        buy_signals, sell_signals = self.signal_arrays(
        ...
        scores = np.asarray(self.score_signals(params, indicator_matrix), dtype=float)
    ...
    def parameter_schema(self):
        """Typed solver contract adapted from compact ParamDim declarations."""
        from ..search.contracts import ParameterSchema

        return ParameterSchema.from_param_space(self.param_space)

    def sample_params(self, rng=None) -> Params:
        """Sample the canonical schema for previews and contract tests."""
        generator = rng or __import__("random").Random()
        return Params(
            values=dict(self.parameter_schema.sample(generator)),
            _engine=self.name,
        )
    ...
    def scan_today(
        ...
        try:
            ...
            from ..backtest.engine import _build_indicator_matrix
...
class ArraySignalStrategy(TradingStrategy):
    """Convenience base for strategies expressed as score and signal arrays."""

    @abstractmethod
    def score_signals(
        self, params: Params, indicator_matrix: np.ndarray
    ) -> np.ndarray:
        """Return a ``(date, symbol, buy/sell)`` priority tensor."""
        ...
    ...
    def signal_arrays(
        ...
        """Return boolean buy and sell matrices."""
        ...

    def make_signals(
        self, params: Params, market_data: StrategyMarketData
    ) -> TradePlan:
        return self._make_array_plan(params, market_data)
